...
fun PantallaListaSintomas(
    ...
    LaunchedEffect(estado.userId) {
        if (estado.userId.isNotBlank()) {
            viewModel.cargarSintomas(estado.userId)
        }
    }
...
fun TarjetaSintoma(
    ...
    if (mostrarDialogo) {
        AlertDialog(
            ...
            text = { Text("¿Estás seguro de que quieres eliminar ${sintoma.nombre}?") },
            confirmButton = {
                TextButton(onClick = {
                    onClickEliminar()
    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = "Intensidad: ${sintoma.intensidad}/10",
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
                Text(
                    text = "${sintoma.fecha} a las ${sintoma.hora}",
