...
fun PantallaFormularioDoctor(
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(value = nombre, onValueChange = { nombre = it; errorNombre = false },
                label = { Text("Nombre del doctor *") }, modifier = Modifier.fillMaxWidth(),
                isError = errorNombre, supportingText = if (errorNombre) {{ Text("El nombre es obligatorio") }} else null)

            OutlinedTextField(value = especialidad, onValueChange = { especialidad = it; errorEspecialidad = false },
                label = { Text("Especialidad *") }, modifier = Modifier.fillMaxWidth(),
                isError = errorEspecialidad, supportingText = if (errorEspecialidad) {{ Text("La especialidad es obligatoria") }} else null)

            OutlinedTextField(value = telefono, onValueChange = { telefono = it; errorTelefono = false },
                label = { Text("Teléfono *") }, modifier = Modifier.fillMaxWidth(),
                isError = errorTelefono, supportingText = if (errorTelefono) {{ Text("El teléfono es obligatorio") }} else null)

            OutlinedTextField(value = email, onValueChange = { email = it },
                label = { Text("Email (opcional)") }, modifier = Modifier.fillMaxWidth())

            OutlinedTextField(value = direccion, onValueChange = { direccion = it; errorDireccion = false },
                label = { Text("Dirección *") }, modifier = Modifier.fillMaxWidth(),
                isError = errorDireccion, supportingText = if (errorDireccion) {{ Text("La dirección es obligatoria") }} else null)

            OutlinedTextField(value = notas, onValueChange = { notas = it },
                label = { Text("Notas (opcional)") }, modifier = Modifier.fillMaxWidth(), minLines = 3)
            ...
            Button(
                onClick = {
                    ...
                    if (!errorNombre && !errorEspecialidad && !errorTelefono && !errorDireccion) {
                        val doctor = Doctor(
                            id = doctorId ?: 0, nombre = nombre,
                            especialidad = especialidad, telefono = telefono,
                            email = email.ifBlank { null }, direccion = direccion,
