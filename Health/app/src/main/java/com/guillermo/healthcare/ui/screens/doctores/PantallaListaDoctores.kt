...
fun PantallaListaDoctores(
    ...
    LaunchedEffect(estado.userId) {
        if (estado.userId.isNotBlank()) {
            viewModel.cargarDoctores(estado.userId)
        }
    }
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = { navController.navigate(Pantalla.FormularioDoctor.crearRuta()) }) {
        ...
        if (doctores.isEmpty()) {
            Box(modifier = Modifier.fillMaxSize().padding(paddingValues), contentAlignment = Alignment.Center) {
                Text("No hay doctores. ¡Añade uno!")
            }
        } else {
            LazyColumn(
                ...
                items(doctores) { doctor ->
                    TarjetaDoctor(
                        doctor = doctor,
                        onClickItem = { navController.navigate(Pantalla.DetalleDoctor.crearRuta(doctor.id)) },
                        onClickEliminar = { viewModel.eliminarDoctor(doctor) }
                    )
                }
            }
        }
